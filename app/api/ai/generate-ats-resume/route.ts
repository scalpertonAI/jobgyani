...
import { generateATSResume } from '@/lib/openai';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Generating ATS resume for:', file.name);
    ...
    // Generate ATS-formatted resume
    const result = await generateATSResume(resumeText);

    return NextResponse.json({ success: true, result });
  } catch (error: any) {
    console.error('Error generating ATS resume:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to generate ATS resume' },
      { status: 500 }
    );
  }
}
